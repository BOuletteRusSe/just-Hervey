...
async def Vdm(ctx):
    ...
    while True:
        try:
            ...
            response = requests.get("https://www.viedemerde.fr/aleatoire")
            soup = BeautifulSoup(response.text, "html.parser")

            article = soup.find('article')
            by = article.find("p", {"class": "text-blue-300 text-sm mt-2"}).text
            content = article.find("a", {"class": "block text-blue-500 my-4"})
            ...
            embed = discord.Embed(title="**just Hervey 💎 || VDM 💀**", color=0x0e9a98, description=content_text, url=article_url)
